...
myApp.controller('myController', function ($scope, $http) {
    ...
    $http.get('./PHP/connect.php').then(function (res) {
        ...
        array_clone = loop(0, array.length);
        $scope.get_all_cart();
    }, function () {
        alert("Lỗi kết nối dữ liệu!!!");
    });
    ...
    $scope.next = function () {
        if (star < array_clone.length && star + jump < array_clone.length) {
            star = end;
            if (end + jump > array_clone.length) {
                end = array_clone.length;
            } else {
                end += jump;
            }
            $scope.text += 1;
