...
myApp.controller('myController', function ($scope, $http) {
    ...
    $http.get(`./PHP/get_bill.php?name_accound=${sessionStorage.getItem("my_account_user")}`).then(function (res) {
        $scope.sum = 0;
        array_bill = res.data.cart;
        document.getElementById("list_null2").innerHTML = "Chưa có sản phẩm nào trong giỏ hàng!";
        document.getElementById("footer_all").style.paddingTop = "310px";
        try {
            $scope.get_cart();
            if (end > array_bill.length) {
                $scope.products = res.data.cart;
                document.getElementById("list_null2").innerHTML = "";
                document.getElementById("footer_all").style.paddingTop = "310px";
            } else {
                $scope.products = res.data.cart;
                document.getElementById("list_null2").innerHTML = "";
