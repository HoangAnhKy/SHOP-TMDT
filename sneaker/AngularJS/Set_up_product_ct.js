...
var id_img = sessionStorage.getItem("storageID");
var name_img = sessionStorage.getItem("storageName");
myApp.controller('myController', function ($scope, $http) {
    ...
    $http.get('./PHP/get_image_product.php').then(function (res) {
        array_product = res.data.image;
        array_img = loop_img(array_product, id_img);
        $scope.Main_image = array_img[0].image;
        $scope.product_img = array_img;
        $scope.get_all_cart();
    }, function () {
        alert("Lỗi kết nối dữ liệu!!!");
    });

    $scope.seen_image = function (src) {
    ...
    $http.get('./PHP/connect.php').then(function (res) {
        ...
        $scope.products = loop_img(array, name_img);
        // sản phẩm tương tự
        $scope.products_same = loop_same(star, array.length, id_img, name_img);
    }, function () {
        alert("Lỗi kết nối dữ liệu!!!");
    });
    // load yêu thích
    ...
    $scope.get_cart = function (sp) {
        ...
        if ($scope.len_cart == 0) {
            // $scope.add_cart(sp);
            $scope.add(sp, `./PHP/Insert_cart.php`);
        } else {
...
function loop_same(star, end, id, name) {
    var array_clone = [];
    count = 0;
    for (var i = star; i < end; ++i) {
        if (array[i].name != name && array[i].id == id && count < 5) {
            count++;
            array_clone.push(array[i]);
        }
    }
